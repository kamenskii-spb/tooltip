export default class Tooltip {
  ...
  open(contentHTML = "") {
    if (document.readyState === "complete" && !this.tooltipOpen) {
      ...
      new Error("tooltip already open");
    }
  }
...
function _createArrow(options) {
  ...
  $arrow.classList.add(
    "km-tooltip-arrow",
    `km-tooltip-arrow-${options.position}`
  );
...
return ["left", "right"].includes(options.arrowPosition) &&
       ["top", "bottom"].includes(options.position)
}

function _is_setArrowPositionY(options){
  return ["top", "bottom"].includes(options.arrowPosition) &&
         ["left", "right"].includes(options.position)
  }